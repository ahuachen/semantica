...
export function KGOverviewTab() {
  ...
  const fetchOverview = useCallback(async () => {
    ...
    try {
      ...
      if (statsRes.status === 207) {
        setError((statsData as any).message || t('kgOverview.statsPartialWarning'));
      }
      ...
      if (nodesRes.status === 207) {
        const nodesMessage = (nodesData as any).message || t('kgOverview.nodesPartialWarning');
      ...
      setError(err instanceof Error ? err.message : t('kgOverview.loadFailedError'));
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    async function fetchInitial() {
      ...
      try {
        ...
        if (!ignore) {
          setStats(statsData);
          if (statsRes.status === 207) {
            setError((statsData as { message?: string }).message || t('kgOverview.statsPartialWarning'));
          }
        }
        ...
        if (!ignore) {
          setNodeTypeMap(buildTypeMap(nodes, "type"));
          if (nodesRes.status === 207) {
            const nodesMessage = (nodesData as { message?: string }).message || t('kgOverview.nodesPartialWarning');
            setError((prev) => (prev ? `${prev} ${nodesMessage}` : nodesMessage));
          }
        }
        ...
        if (!ignore) setError(err instanceof Error ? err.message : t('kgOverview.loadFailedError'));
      } finally {
  ...
  const statCards = [
    { label: t('kgOverview.nodesLabel'),   value: totalNodes.toLocaleString(), color: "var(--ws-accent)",  sub: `${nodeTypeEntries.length} ${t('kgOverview.typesSubtitle')}` },
    { label: t('kgOverview.edgesLabel'),   value: totalEdges.toLocaleString(), color: "var(--ws-green)",   sub: `${edgeTypeEntries.length} ${t('kgOverview.relationshipTypesSubtitle')}` },
    { label: t('kgOverview.densityLabel'), value: totalNodes > 1 ? ((totalEdges / (totalNodes * (totalNodes - 1))) * 100).toFixed(3) + "%" : "—", color: "var(--ws-purple)", sub: t('kgOverview.graphDensitySubtitle') },
  ];
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 22px", borderBottom: "1px solid var(--ws-border)", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div className="ws-body" style={{ fontSize: 11, marginTop: 2 }}>{t('kgOverview.description')}</div>
          </div>
        </div>
      ...
      <div className="ws-scroll" style={{ flex: 1, padding: "18px 22px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <div className="ws-card" style={{ padding: "16px 18px", gap: 8, display: "flex", flexDirection: "column" }}>
            <div className="ws-eyebrow" style={{ marginBottom: 4 }}>{t('kgOverview.nodeTypeBreakdown')}</div>
            {loading ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {[80, 65, 45, 35, 25].map((w, i) => <div key={i} className="ws-skeleton" style={{ height: 10, width: `${w}%` }} />)}
              </div>
            ) : nodeTypeEntries.length === 0 ? (
              <div className="ws-body" style={{ fontSize: 12 }}>{t('kgOverview.noData')}</div>
            ) : (
          ...
          <div className="ws-card" style={{ padding: "16px 18px", gap: 8, display: "flex", flexDirection: "column" }}>
            <div className="ws-eyebrow" style={{ marginBottom: 4 }}>{t('kgOverview.edgeTypeBreakdown')}</div>
            {loading ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {[70, 55, 48, 30, 20].map((w, i) => <div key={i} className="ws-skeleton" style={{ height: 10, width: `${w}%` }} />)}
              </div>
            ) : edgeTypeEntries.length === 0 ? (
              <div className="ws-body" style={{ fontSize: 12 }}>{t('kgOverview.edgeTypeHint')}</div>
            ) : (
        ...
        {topNodes.length > 0 && (
          <div className="ws-card" style={{ padding: "16px 18px" }}>
            <div className="ws-eyebrow" style={{ marginBottom: 12 }}>{t('kgOverview.topConnectedNodes')}</div>
