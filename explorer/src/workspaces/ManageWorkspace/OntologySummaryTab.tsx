    ...
    <div style={schemeCardStyle}>
      ...
      >
        ...
        <span style={{ color: "#6a7f97", fontSize: 11 }}>
          {isLoading ? "…" : t('ontologySummary.conceptCount', { count: totalConcepts })}
        </span>
      </button>
      ...
      {expanded ? (
        <div style={{ paddingTop: 4, paddingBottom: 8 }}>
          {isLoading ? (
            <div style={{ padding: "8px 24px", color: "#6a7f97", fontSize: 12 }}>{t('ontologySummary.loadingConcepts')}</div>
          ) : hierarchy.length === 0 ? (
            <div style={{ padding: "8px 24px", color: "#6a7f97", fontSize: 12, fontStyle: "italic" }}>
              {t('ontologySummary.noConceptsFound')}
            </div>
          ) : (
    ...
    <div style={shellStyle}>
      ...
      <div style={headerStyle}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ color: "#ebf3ff", fontSize: 16, fontWeight: 700 }}>{t('ontologySummary.title')}</div>
            <div style={{ color: "#8b949e", fontSize: 12 }}>
              {isLoading
                ? t('ontologySummary.loadingSchemes')
                : t('ontologySummary.vocabularySchemeCount', { count: schemes.length }) + ' ' + t('ontologySummary.loaded')}
            </div>
          </div>
        </div>
        {onOpenVocabularyBrowser ? (
          <button onClick={onOpenVocabularyBrowser} style={openBrowserBtnStyle}>
            ...
            <span>{t('ontologySummary.openBrowser')}</span>
          </button>
        ) : null}
      </div>
        ...
        <div style={treeColumnStyle}>
          {isLoading ? (
            ...
            <div style={emptyStateStyle}>
              ...
              <div style={{ color: "#8b949e", fontSize: 13, marginTop: 12 }}>{t('ontologySummary.noVocabulariesLoaded')}</div>
              <div style={{ color: "#6a7f97", fontSize: 12, marginTop: 4, textAlign: "center", maxWidth: 240 }}>
                {t('ontologySummary.noVocabulariesDesc')}
              </div>
            </div>
          ) : (
          ...
          <div style={detailPanelStyle}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
              <div style={{ color: "#d2a8ff", fontSize: 11, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase" }}>
                {t('ontologySummary.conceptDetail')}
              </div>
            ...
            {selectedConcept.notation ? (
              <div style={{ color: "#8b949e", fontSize: 12, marginBottom: 8 }}>{t('ontologySummary.notationLabel', { notation: selectedConcept.notation })}</div>
            ) : null}
            ...
            {selectedConcept.description ? (
              <div style={detailSectionStyle}>
                <div style={detailLabelStyle}>{t('ontologySummary.description')}</div>
            ...
            {selectedConcept.alt_labels?.length ? (
              <div style={detailSectionStyle}>
                <div style={detailLabelStyle}>{t('ontologySummary.alternativeLabels')}</div>
            ...
            {(selectedConcept.children?.length ?? 0) > 0 ? (
              <div style={detailSectionStyle}>
                <div style={detailLabelStyle}>{t('ontologySummary.narrowerConceptsCount', { count: selectedConcept.children!.length })}</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  ...
                  {selectedConcept.children!.length > 8 ? (
                    <div style={{ color: "#6a7f97", fontSize: 11 }}>{t('ontologySummary.moreCount', { count: selectedConcept.children!.length - 8 })}</div>
                  ) : null}
                </div>
              </div>
            ) : null}
