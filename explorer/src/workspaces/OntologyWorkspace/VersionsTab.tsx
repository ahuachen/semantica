  ...
  const loadVersions = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setVersions(data);
        if (response.status === 207) setError(data.message || t("versions.partialLoadVersions"));
      } else {
        setError(t("versions.loadVersionsFailedStatus", { status: response.status }));
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : t("versions.loadVersionsError"));
    }
  }, [ontologyUri, t]);
  ...
  const loadProposals = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setProposals(data);
        if (response.status === 207) setError(data.message || t("versions.partialLoadProposals"));
      } else {
        setError(t("versions.loadProposalsFailedStatus", { status: response.status }));
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : t("versions.loadProposalsError"));
    }
  }, [t]);
    ...
    async function fetchInitial() {
      ...
      try {
        ...
        if (propRes.ok) {
          ...
          if (!ignore) {
            ...
            if (propRes.status === 207) setError(propData.message || t("versions.partialLoadProposals"));
          }
        } else if (!ignore) {
          setError(t("versions.loadProposalsFailedStatus", { status: propRes.status }));
        }
      } catch (err) {
        if (!ignore) setError(err instanceof Error ? err.message : t("versions.loadProposalsError"));
      }
      ...
      try {
        ...
        if (verRes.ok) {
          ...
          if (!ignore) {
            setVersions(verData);
            if (verRes.status === 207) setError(verData.message || t("versions.partialLoadVersions"));
          }
        } else if (!ignore) {
          setError((prev) => prev || t("versions.loadVersionsFailedStatus", { status: verRes.status }));
        }
      } catch (err) {
        if (!ignore) setError((prev) => prev || (err instanceof Error ? err.message : t("versions.loadVersionsError")));
      }
    }
    void fetchInitial();
    return () => { ignore = true; };
    ...
    try {
      ...
      if (response.ok) {
        if (response.status === 207) {
          ...
          alert(t("versions.approvedAlert"));
        }
        loadProposals();
      } else {
        setError(t("versions.approveFailedStatus", { status: response.status }));
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : t("versions.approveError"));
    }
  }, [loadProposals, t]);
    ...
    try {
      ...
      if (response.ok) {
        if (response.status === 207) {
          ...
          alert(t("versions.rejectedAlert"));
        }
        loadProposals();
      } else {
        setError(t("versions.rejectFailedStatus", { status: response.status }));
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : t("versions.rejectError"));
    }
  }, [loadProposals, t]);
    ...
    try {
      ...
      if (response.ok) {
        if (response.status === 207) {
          ...
          alert(t("versions.publishedAlert"));
        }
        loadProposals();
        loadVersions();
      } else {
        setError(t("versions.publishFailedStatus", { status: response.status }));
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : t("versions.publishError"));
    }
  }, [loadProposals, loadVersions, t]);
  ...
  const runVersionComparison = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(t("versions.compareFailedStatus", { status: response.status }));
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : t("versions.compareError"));
    } finally {
      setIsLoading(false);
    }
  }, [comparePair, ontologyUri, t]);
    ...
    <div style={containerStyle}>
        ...
        <div style={modalOverlayStyle} onClick={() => setShowProposalModal(false)}>
          <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "16px" }}>
              <h3 style={{ margin: 0, color: "#ebf3ff", fontSize: "16px" }}>{t("versions.proposalDetailsTitle")}</h3>
            ...
            <div style={{ marginBottom: "12px" }}>
              <label style={{ display: "block", color: "#8fa8c6", fontSize: "12px", marginBottom: "4px" }}>
                {t("versions.impactAnalysisLabel")}
              </label>
            ...
            <div style={{ marginBottom: "12px" }}>
              <label style={{ display: "block", color: "#8fa8c6", fontSize: "12px", marginBottom: "4px" }}>
                {t("versions.shaclValidationLabel")}
              </label>
              <pre style={{ background: "rgba(3, 9, 18, 0.8)", padding: "12px", borderRadius: "6px", color: "#ebf3ff", fontSize: "12px", overflow: "auto" }}>
            ...
            <div>
              <label style={{ display: "block", color: "#8fa8c6", fontSize: "12px", marginBottom: "4px" }}>
                {t("versions.commentsLabel", { count: selectedProposal.comments.length })}
              </label>
      ...
      {showCompareModal && comparePair && (
        <div style={modalOverlayStyle} onClick={() => setShowCompareModal(false)}>
          <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "16px" }}>
              <h3 style={{ margin: 0, color: "#ebf3ff", fontSize: "16px" }}>{t("versions.compareVersionsTitle")}</h3>
            ...
            <button style={buttonStyle} onClick={runVersionComparison} disabled={isLoading}>
              <Scale size={12} />
              {isLoading ? t("versions.comparing") : t("versions.compareButton")}
            </button>
            {compareResult && (
