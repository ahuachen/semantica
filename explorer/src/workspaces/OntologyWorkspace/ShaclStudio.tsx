...
export function ShaclStudio({ onJumpToNode }: ShaclStudioProps) {
  ...
  useEffect(() => {
    ...
    loadOntologyRegistry()
      ...
      .catch((err) => {
        if (cancelled) return;
        setError(err instanceof Error ? err.message : t("shacl.loadRegistryError"));
      });
    return () => {
      cancelled = true;
    };
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("shacl.generateError"));
    } finally {
      setLoading(false);
    }
  }, [selectedUri, t]);
  ...
  const handleValidate = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("shacl.validateError"));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={pageStyle}>
      <section style={heroStyle}>
        ...
        <div style={selectorShellStyle}>
          <label style={labelStyle}>{t("shacl.ontologyLabel")}</label>
      ...
      <div style={gridStyle}>
        <section style={cardStyle}>
          <div style={panelHeaderStyle}>
            <h3 style={sectionTitleStyle}>{t("shacl.shapeLibraryTitle")}</h3>
        ...
        <section style={editorShellStyle}>
          <div style={panelHeaderStyle}>
            <h3 style={sectionTitleStyle}>
              {selectedShapeId ? selectedShapeId : t("shacl.defaultEditorTitle")}
            </h3>
      ...
      <section style={cardStyle}>
        <div style={panelHeaderStyle}>
          <h3 style={sectionTitleStyle}>{t("shacl.validationReportTitle")}</h3>
        ...
        {validation ? (
          ...
          <p style={mutedStyle}>{t("shacl.validatePrompt")}</p>
        )}
      </section>
    </div>
  );
