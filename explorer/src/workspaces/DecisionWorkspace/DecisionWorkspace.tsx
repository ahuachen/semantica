...
function OutcomeBadge({ outcome }: { outcome: OutcomeKind }) {
  ...
  return (
    <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: 999, fontSize: 10, fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase", color: c.color, background: c.bg, border: `1px solid ${c.border}` }}>
      {outcome || t('decision.unknownOutcome')}
    </span>
  );
}
...
function CausalFlow({ chain, loading }: { chain: ChainStep[]; loading: boolean }) {
  ...
  if (!chain.length) return (
    <div className="ws-empty">
      ...
      <div className="ws-empty-title">{t('decision.noChainSteps')}</div>
      <div className="ws-empty-body">{t('decision.noChainStepsDesc')}</div>
    </div>
  );
  return (
  ...
  useEffect(() => {
    ...
    fetch("/api/decisions", { signal: ctrl.signal })
      .then(async (r) => {
        ...
        if (r.status === 207) setError(data.message || t('decision.partialWarningDecisions'));
        return data;
      })
      .then((data) => {
      ...
      .catch((e) => {
        if (e?.name !== "AbortError") {
          setError(e instanceof Error ? e.message : t('decision.loadDecisionsError'));
        }
      })
      .finally(() => {
  ...
  async function loadChain(d: { decision_id: string; category?: string; outcome?: string }) {
    ...
    try {
      ...
      if (res.status === 207 && !ctrl.signal.aborted) {
        setError(data.message || t('decision.partialWarningChain'));
      }
      ...
      <div className="ws-sidebar" style={{ width: 290 }}>
        <div className="ws-sidebar-header">
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            ...
            <div style={{ color: "var(--ws-text)", fontSize: 14, fontWeight: 700 }}>{t('decision.decisionsTitle')}</div>
        ...
        <div className="ws-sidebar-body">
          {listLoading ? (
            ...
            <div className="ws-empty" style={{ padding: "28px 12px" }}>
              ...
              <div className="ws-empty-body">{decisions.length === 0 ? t('decision.noDecisionsAvailable') : t('decision.adjustFilter')}</div>
            </div>
          ) : (
        ...
        {selected ? (
            ...
            <div className="ws-card" style={{ padding: 22 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20 }}>
                ...
                {chain.length > 0 && !chainLoading && (
                  <span className="ws-pill ws-pill--accent" style={{ marginLeft: "auto" }}>{t('decision.stepCount', { count: chain.length })}</span>
                )}
              </div>
          ...
          <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", zIndex: 1 }}>
            <div className="ws-empty">
              ...
              <div className="ws-empty-title">{t('decision.noDecisionSelected')}</div>
              <div className="ws-empty-body">{t('decision.selectDecisionDesc')}</div>
            </div>
          </div>
        )}
      </div>
