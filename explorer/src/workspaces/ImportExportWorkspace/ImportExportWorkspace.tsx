...
export function ImportExportWorkspace() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      showToast("success", t('importExport.exportReadyToast', { fileName: `semantica_export.${exportFormat}` }));
      ...
      <div className="ws-padded" style={{ display: "flex", flexDirection: "column", gap: 24, maxWidth: 980, margin: "0 auto", width: "100%" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            ...
            <div className="ws-body" style={{ marginTop: 2 }}>{t('importExport.description')}</div>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
          ...
          <div className="ws-card" style={{ padding: 24, display: "flex", flexDirection: "column", gap: 18 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <div style={{ color: "var(--ws-text)", fontWeight: 700, fontSize: 14 }}>{t('importExport.importEntitiesAndRelations')}</div>
            </div>
            ...
            >
              ...
              {file ? (
                <>
                  ...
                  <div className="ws-body" style={{ fontSize: 11 }}>{t('importExport.fileSizeHint', { size: (file.size / 1024).toFixed(1) })}</div>
                </>
              ) : (
                <>
                  <UploadCloud size={36} color="var(--ws-accent)" style={{ opacity: 0.7 }} />
                  <div style={{ color: "var(--ws-text)", fontWeight: 600 }}>{t('importExport.dragAndDrop')}</div>
            ...
            >
              {isUploading ? <><Loader2 size={15} className="ws-spin" />{t('importExport.uploading')}</> : <><UploadCloud size={15} />{t('importExport.uploadToGraph')}</>}
            </button>
          </div>
          ...
          <div className="ws-card" style={{ padding: 24, display: "flex", flexDirection: "column", gap: 18 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <div style={{ color: "var(--ws-text)", fontWeight: 700, fontSize: 14 }}>{t('importExport.exportGraphSnapshot')}</div>
            </div>
            ...
            <div style={{ flex: 1, padding: "14px 16px", borderRadius: "var(--ws-radius-sm)", background: "rgba(0,0,0,0.22)", border: "1px solid var(--ws-border)" }}>
              ...
              <div className="ws-body" style={{ fontSize: 12 }}>
                {exportFormat === "json"
                  ? t('importExport.jsonDescription')
            ...
            >
              {isExporting ? <><Loader2 size={15} className="ws-spin" />{t('importExport.preparing')}</> : <><Download size={15} />{t('importExport.downloadExport')}</>}
            </button>
          </div>
        </div>
      </div>
