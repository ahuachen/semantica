...
const TEMPLATES: { labelKey: string; query: string }[] = [
  { labelKey: "sparql.templateAllTriples", query: "SELECT ?s ?p ?o\nWHERE {\n  ?s ?p ?o\n}\nLIMIT 20" },
  { labelKey: "sparql.templateNodeTypes", query: "SELECT ?type (COUNT(?s) AS ?count)\nWHERE {\n  ?s a ?type\n}\nGROUP BY ?type\nORDER BY DESC(?count)" },
  { labelKey: "sparql.templateOutgoingEdges", query: "SELECT ?predicate ?object\nWHERE {\n  <urn:node:example> ?predicate ?object\n}\nLIMIT 50" },
  { labelKey: "sparql.templatePathBetween", query: "SELECT ?mid ?p1 ?p2\nWHERE {\n  <urn:node:a> ?p1 ?mid .\n  ?mid ?p2 <urn:node:b>\n}\nLIMIT 20" },
];
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
        ...
        <div style={{ padding: "10px 16px", borderBottom: "1px solid var(--ws-border)", display: "flex", alignItems: "center", gap: 8, flexShrink: 0, background: "rgba(0,0,0,0.18)" }}>
          <div style={{ display: "flex", gap: 6, flex: 1, flexWrap: "wrap" }}>
            <span className="ws-eyebrow" style={{ alignSelf: "center", marginRight: 4 }}>{t('sparql.templates')}</span>
            {TEMPLATES.map((tpl) => (
              <button
                key={tpl.labelKey}
                className="ws-btn ws-btn--ghost"
                style={{ padding: "4px 10px", fontSize: 11 }}
                onClick={() => { setQuery(tpl.query); setResult(null); }}
              >
          ...
          >
            {isLoading
              ...
              : <><Play size={13} />{t('sparql.runQuery')}</>}
          </button>
        </div>
          ...
          <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column", overflow: "hidden", background: "rgba(0,0,0,0.12)" }}>
            <div style={{ padding: "10px 16px", borderBottom: "1px solid var(--ws-border)", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 7, color: "var(--ws-text-muted)", fontSize: 13, fontWeight: 700 }}>
                <Table2 size={14} />
                {t('sparql.results')}
                {result?.rows && <span className="ws-pill ws-pill--accent">{result.rows.length} {t('sparql.rows')}</span>}
              </div>
            ...
            <div className="ws-scroll" style={{ flex: 1 }}>
                ...
                <div className="ws-animate-in" style={{ overflowX: "auto" }}>
                  {result.rows.length === 0 ? (
                    <div className="ws-empty">
                      ...
                      <div className="ws-empty-body">{t('sparql.noResultsDesc')}</div>
                    </div>
                  ) : (
                              ...
                              <td key={c} style={{ padding: "7px 14px", maxWidth: 300, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontFamily: "monospace" }} title={String(r[c] ?? "")}>
                                {r[c] != null ? (
                                  String(r[c]).startsWith("urn:") || String(r[c]).startsWith("http")
                                    ...
                                    : String(r[c])
                                ) : <span style={{ color: "var(--ws-text-dim)", fontStyle: "italic" }}>{t('sparql.nullValue')}</span>}
                              </td>
                            ))}
                          </tr>
                        ))}
              ...
              {!result && !isLoading && (
                <div className="ws-empty">
                  ...
                  <div className="ws-empty-title">{t('sparql.readyToQuery')}</div>
                  <div className="ws-empty-body">{t('sparql.readyToQueryDesc')}</div>
                </div>
              )}
            </div>
          </div>
